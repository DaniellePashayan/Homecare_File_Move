...
class Folder:
    ...
    def convert_files_to_df(date:datetime.datetime, file_list:list):
    ...
    def copy_files_to_destination(self):
        ...
        for file in file_list:
            source_file = os.path.join(dated_folder, file)
            destination_file = os.path.join(self.destination_path, file)
            try:
                # shutil.copy(source_file, destination_file)
                # check if file exists in destination
                if os.path.exists(destination_file):
                    file_df.loc[file_df['file'] == file, 'moved'] = True
                files_moved += 1
                logger.info(f'Copied {file} to {self.destination_path}')
